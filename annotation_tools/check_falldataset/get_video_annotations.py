...
def generate_annotations(data_path, vidieo_type='.avi'):
    try:
        ...
        for ann_txt in tqdm(txt_files, desc=data_path, total=num_txt):
            ...
            video_file = video_path.joinpath(ann_txt.stem + vidieo_type)
            ...
            for frame_id, fall_direction, x1, y1, x2, y2 in frame_info:
                if cap.isOpened() and frame_id < frame_count:
                    ...
                    if xyxy != []:  # get more exact bbox
                        ...
                        img = cv2.rectangle(img, _int(x1, y1), _int(x2, y2), color=(255, 255, 0), thickness=1)
...
if __name__ == '__main__':
    ...
    for data_root in train_data_roots:
        generate_annotations(data_root, vidieo_type='.mp4')