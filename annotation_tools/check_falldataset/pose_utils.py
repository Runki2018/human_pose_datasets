...
def vis_annotations(img, bbox, kpts, size=2, thr=0.5):
    kpts = np.array(kpts).reshape((-1, 3))
    # points
    for cx, cy, vis in kpts:
        color = (0, 255, 0) if vis > thr else (0, 0, 255)
        cv2.circle(img, (int(cx), int(cy)), size*3, color, cv2.FILLED)
    # skeleton
    for pair, color in zip(pose_structure['skeleton'], pose_structure['line_color']):
        # line(img, pt1, pt2, color[, thickness[, lineType[, shift]]]) -> img
        pt1 = int(kpts[pair[0], 0]), int(kpts[pair[0], 1])
        pt2 = int(kpts[pair[1], 0]), int(kpts[pair[1], 1])
        cv2.line(img, pt1, pt2, color, thickness=size)
    # bbox 
    bbox = [int(x) for x in bbox]
    left_top = bbox[:2]
    right_bottom = bbox[0] + bbox[2], bbox[1] + bbox[3]
    # rectangle(img, pt1, pt2, color[, thickness[, lineType[, shift]]]) -> img
    cv2.rectangle(img, left_top, right_bottom, (0, 255, 0), thickness=size)  
    return img
